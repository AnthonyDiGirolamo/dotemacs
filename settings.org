...
** dired
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package dired
      ...
      (define-key dired-mode-map (kbd "N") 'dired-next-dirline) ;; colemak
      (define-key dired-mode-map (kbd "E") 'dired-prev-dirline) ;; colemak
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (use-package eshell
      ...
      (defun eshell/d ()
        (interactive)
        (let* ((dirs
                (reverse (cl-loop for d in (append (cdr (cdr eshell-last-dir-ring)) nil)
                                  when d
                                  collect d))))
          ;; (s-join "\n" (reverse
          ;;               (-map-indexed
          ...
          (ivy-read
           ...
           (cl-loop for index
                    from 0
                    for dir in dirs
                    collect (cons (format "%-4s %-30s" (propertize (format "%d" index) 'face 'font-lock-builtin-face) dir) index))
           :action (lambda (arg) (eshell/cd (format "-%s" (cdr arg))))
           )
          ))
...
* emacs and git on windows
  ...
  #+BEGIN_SRC fundamental :tangle (if (eq system-type 'windows-nt) "C:/Users/anthony/runemacs.bat" "no")
    ...
    SET PATH=%GIT_HOME%\bin;%GIT_HOME%\mingw\bin;%GIT_HOME%\cmd;%GIT_HOME%\usr\bin;%PATH%
    SET HOME=.\
