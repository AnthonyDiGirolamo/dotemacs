...
* amd/functions
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (defun run-pico8 (args)
       "run a pico-8 cartridge then revert buffer"
       (interactive "P")
       (let ((current-file-path (buffer-file-name (current-buffer))))
         (save-buffer)
         (shell-command (concat "/usr/lib/pico8/pico8 -run " current-file-path))
         (revert-buffer nil t)))
