...
* ivy, swiper, and counsel
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (use-package counsel
      ...
      :config
      (defvar counsel-calc-units-map (make-sparse-keymap))
      ...
      (defun counsel-calc-units ()
        ...
        (let ((this-command 'counsel-calc-units))
          (ivy-read
           ...
           (mapcar
            (lambda (unit-list)
              (let* ((command (first unit-list))
                     (key (symbol-name command))
                     (doc (delq nil
                                (mapcar
                                 (lambda (item)
                                   (when (stringp item) item))
                                 (rest unit-list))))
                     (doc-size (length doc))
                     (definition (cond ((eq doc-size 1) key)
                                       ((eq doc-size 2) (first doc))
                                       ((eq doc-size 3) (last doc))))
                     (description (cond ((eq doc-size 1) (format "%s (base unit)" (last doc)))
                                        (t               (second doc) ))))
                (cons (format "%-12s %-30s %s"
                              (propertize key 'face 'font-lock-builtin-face)
                              definition
                              description)
                      key)))
            math-standard-units)
           ;; TODO add math-unit-prefixes
           :action #'calc-alg-entry
           :keymap counsel-calc-units-map)))
    )
  #+END_SRC
